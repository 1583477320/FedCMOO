...
class Server(object):
    ...
    def __init__(self, config):
        ...
        if self.config['reload_exp']['flag'] is True:
          ...
          self.config['paths']['experiment_history'],self.config['reload_exp']['folder_name'], 'exp_config.json')
        ...
        if self.config['wandb']['flag']:
            ...
            self.wandb.init(project=self.config['wandb']['wandb_project_name'], name=self.config['wandb']['wandb_runname'])
    ...
    def boot(self, use_the_same_dataset_clients = False):
        ...
        try:
            self.experiment_module = getattr(__import__(experiment_module_name+'.'+experiment_module_name), experiment_module_name)
        ...
        logging.info(f"You can find experiment results in the folder: {self.config['paths']['experiment_history']}/{self.metrics.exp_id}")
        ...
        self.model_cuda = {k:copy.deepcopy(self.model[k]).to('cuda') for k in self.model.keys()} if self.boost_w_gpu else None
        ...
        if self.experiment_reloaded:
            ...
            if self.config["hyperparameters"]["local_training"]["local_lr_scheduler_flag"]: # LR scheduler
                self.config["hyperparameters"]["local_training"]["initial_local_lr"] *= (0.5 ** (self.metrics.current_round // (self.config['max_round'] // 5)))
        ...
        if self.config['algorithm'] in ['fedcmoo', 'fedcmoo_pref']:          
            if 'randsvd' in self.config['proposed_approx_method']:
                if 'direct' in self.config['proposed_approx_method']:
                    logging.info(f"To approximate covariance of Jacobian matrix, one-way rand.svd-compressed matrices (with extra {self.config['proposed_approx_extra_upload_d']} d upload) is used.")
                else:
                    logging.info(f"To approximate covariance of Jacobian matrix, our proposed two-way rand.svd-compressed matrices (with extra {self.config['proposed_approx_extra_upload_d']} d upload) is used.")
            elif 'topk' in self.config['proposed_approx_method']:
                if 'direct' in self.config['proposed_approx_method']:
                    logging.info(f"To approximate covariance of Jacobian matrix, one-way top-K-compressed matrices (with extra {self.config['proposed_approx_extra_upload_d']} d upload) is used.")
                else:
                    logging.info(f"To approximate covariance of Jacobian matrix, our proposed two-way top-K-compressed matrices (with extra {self.config['proposed_approx_extra_upload_d']} d upload) is used.")
            else:
    ...
    def train(self):
        ...
        for self.round_num in range(start_round, self.config['max_round']):
            ...
            participating_clients = random.sample(self.clients, min(self.config['nb_of_participating_clients'], len(self.clients)))
            algorithm_specific_log = ''
            if self.round_num == 0:
                if len(participating_clients) < self.config['nb_of_participating_clients']:
                    logging.warning(f"Number of total clients ({self.total_clients}) is less than the desired number of participating clients ({self.config['nb_of_participating_clients']}). All clients are participating!")
                else:
                    logging.warning(f"Out of {self.total_clients} clients {self.config['nb_of_participating_clients']} clients participate every round!")
            ...
            if self.config['algorithm'] == 'fsmgda':
                ...
                for i, client in enumerate(participating_clients):
                    ...
                    averaged_updates = update_average(averaged_updates, updates, self.tasks, 1 / len(participating_clients))
                ...
                algorithm_specific_log += ' Scales: ' + ', '.join([f'{task}: {self.scales[task]:.4f}' for task in self.scales])

                # Aggregate updates to update the global model
                self.aggregate_updates(model_to_aggregate = {True: self.model_cuda, False: self.model}[self.boost_w_gpu], normalized_updates=averaged_updates, scales=self.scales)
                if self.boost_w_gpu:
                ...
                for i, client in enumerate(participating_clients):
                    ...
                    client_scale_update, _ = client.local_train(self.config, 
                                    ...
                                    self.experiment_module, self.tasks, first_local_round=True, current_weight=self.scales, save_to_gpu = save_to_gpu)
                    scale_updates.append(client_scale_update)
                G_T_G_estimate = self.estimateG_T_G(matrices = scale_updates, method = self.config['proposed_approx_method']) # scaling doesn't matter
                # myDel(scale_updates)
                average_loss_np = self.getParticipatingClientLoss(participating_clients)

                if not hasattr(self, 'epo'):
                    preference = np.array([1 for _ in self.tasks]) if self.config['algorithm_args']['fedcmoo_pref']['preference'] == 'uniform' else np.array( self.config['algorithm_args']['fedcmoo_pref']['preference'] )
                    if 'epo_eps' in self.config["exp_identifier"]:
                        temp = float(re.search(r'epo_eps(-?\d+\.?\d*(e[+-]?\d+)?)', self.config["exp_identifier"]).group(1))
                ...
                scalesArr = scalesArr * (1-min_weight_multiplier)+ np.array([min_weight_multiplier/len(self.tasks) for _ in self.tasks])
                    
                for i, alpha in enumerate(scalesArr):
                    self.scales[self.tasks[i]] = alpha
                
                algorithm_specific_log += f'Partc. Client Losses: '+ ', '.join([f'{self.tasks[i]}: {loss:.3f}' for i,loss in enumerate(average_loss_np)])+f' | Scales: ' + ', '.join([f'{task}: {self.scales[task]:.3f}' for task in self.scales])
                ...
                for i, client in enumerate(participating_clients):
                    ...
                    # Apply weighted updates
                    div = 1/(len(participating_clients)*self.config['hyperparameters']['local_training']['nb_of_local_rounds']*self.config['hyperparameters']['local_training']['local_lr'])
                    for key in updates['rep']:
            ...
            if self.config['metrics']['train_period'] > 0 and (self.round_num + 1) % self.config['metrics']['train_period'] == 0:
                ...
                for task in self.tasks:
                    for metric in self.metrics.eval_metrics[task]:
                        average_total_metrics[task][metric] = sum([client_total_metrics[i][task][metric] for i in range(len(participating_clients))]) / len(participating_clients)

                
                # Checkpoint model
                loss_values = [average_total_metrics[t]['loss'] for t in self.tasks if 'loss' in self.metrics.eval_metrics[t]]
                ...
                if self.config['wandb']['flag']:
                    ...
                    for metric in set([metric for task in self.tasks for metric in self.metrics.eval_metrics[task]]):
                        task_values = [average_total_metrics[task][metric] for task in self.tasks if metric in self.metrics.eval_metrics[task]]
            ...
            if self.config['metrics']['val_period'] > 0 and self.val_loader and (self.round_num + 1) % self.config['metrics']['val_period'] == 0:
            ...
            if self.config['metrics']['test_period'] > 0 and (self.round_num + 1) % self.config['metrics']['test_period'] == 0:
            ...
            if self.config['metrics']['model_save_period'] > 0 and (self.round_num + 1) % self.config['metrics']['model_save_period'] == 0:
                logging.info(f'Round {self.round_num + 1}: Models are saved!')
                self.metrics.save_model(self.model)

            logging.info(f'Completed round {self.round_num+1}/{self.config["max_round"]} in {time.time()-starting_time:.1f} seconds! ' + algorithm_specific_log)
    ...
    def create_dataloaders_distribute_data(self):
        ...
        if self.config['experiment'] == "QM9":
            ...
            if self.config['data']['distribution'] != 'uniform':
                logging.info("QM9 experiment is only supported with uniform molecule-wise distribution. Configuration is set to uniform.")
                self.config['data']['distribution'] = 'uniform'
            train_dataset = self.experiment_module.Dataset(split='train', datasetdevice = self.config['data']['trainset_device']).qm9_dataset
        else:
            train_dataset = self.experiment_module.Dataset(split='train', datasetdevice = self.config['data']['trainset_device'],
        ...
        if val_ratio > 0 and val_ratio < 1:
            if self.config["experiment"] in ["MultiMNIST", "CIFAR10_MNIST", "MNIST_FMNIST"]:
                train_dataset, val_dataset = self.experiment_module.split_val_dataset(train_dataset, val_ratio, val_seed = self.config['data']['val_seed'])
            elif self.config['experiment'] in ['CelebA', 'CelebA_CNN', 'CelebA5', "CelebA5_CNN"]:
                val_dataset = self.experiment_module.Dataset(split='val', datasetdevice = self.config['data']['trainset_device'],
                                                     pre_transform =  self.config['data']['pre_transform'])
                val_dataset.split = "test"
                val_dataset.train = False
            elif self.config['experiment'] == 'QM9':
                val_dataset = self.experiment_module.Dataset(split='val', datasetdevice = self.config['data']['trainset_device']).qm9_dataset
        else:
            val_dataset = None

        # Load test data
        if self.config['experiment'] == "QM9":
            test_dataset = self.experiment_module.Dataset(split='test', datasetdevice = self.config['data']['testset_device']).qm9_dataset
        else:
            test_dataset = self.experiment_module.Dataset(split='test', datasetdevice = self.config['data']['testset_device'],
                                                     pre_transform =  self.config['data']['pre_transform'])

        # Create DataLoaders for validation and test datasets
        self.val_loader = self.DataLoader(val_dataset, batch_size=self.config['data']['test_batch_size'], shuffle=False) if val_dataset else None
        self.test_loader = self.DataLoader(test_dataset, batch_size=self.config['data']['test_batch_size'], shuffle=False)
    ...
    def partition_data(self, train_dataset):
        ...
        if distribution == 'uniform':
            ...
            if distribution == 'dirichlet_all_labels':
                ...
                if self.config["experiment"] in ["MultiMNIST", "MNIST_FMNIST"]:
                    ...
                    labels_unique_cpu = np.array([int(''.join(map(str, bits)), 2) for bits in train_dataset.labels[:,[2,20,21]].cpu().numpy()]) # These tasks are selected based on the homogeneoity 
                elif self.config["experiment"] in ["CelebA5", "CelebA5_CNN"]:
                    labels_unique_cpu = np.array([int(''.join(map(str, bits)), 2) for bits in train_dataset.original_labels[:,[2,20,21]].cpu().numpy()])
            else:
            ...
            label_indices_map = {label: np.where(labels_unique_cpu == label)[0].tolist() for label in np.unique(labels_unique_cpu)}
        ...
        for i, client in enumerate(self.clients):
            if self.config['experiment'] == 'QM9':
                client_data = self.experiment_module.QM9Dataset(train_dataset.rawAll[client_indices[i]], [int(i[1:]) for i in self.experiment_module.get_tasks()], device = self.config['data']['trainset_device'])
            else:
    ...
    def aggregate_updates(self, model_to_aggregate, **kwargs):
        if self.config['algorithm'] in ['fsmgda']:
            ...
            for param_name in model_to_aggregate['rep'].state_dict().keys():
                ...
                model_to_aggregate['rep'].state_dict()[param_name].add_(self.config['hyperparameters']['global_lr'] * update_sum)
            ...
            for task in self.tasks:
                for param_name in model_to_aggregate[task].state_dict().keys():
                    if self.config['algorithm_args'][self.config['algorithm']]['scale_decoders']:
                        update_value = self.config['hyperparameters']['global_lr'] * scales[task] * normalized_updates[task][task][param_name]
                    else:
            ...
            for task in self.tasks:
                for param_name in model_to_aggregate[task].state_dict().keys():
                    ...
                    model_to_aggregate[task].state_dict()[param_name].add_(self.config['hyperparameters']['global_lr'] * update_value)
    ...
    def evaluate_metrics(self, phase, client = None):
        ...
        with torch.no_grad():
            ...
            for batch in loader:
                ...
                labels = {task: batch[idx + 1].to(device) for idx, task in enumerate(self.tasks)}# if device != self.config['data'][f'{phase}set_device'] else batch[idx + 1] for idx, task in enumerate(self.tasks)}
        ...
        for client in participating_clients:
            ...
            with torch.no_grad():
                ...
                for batch in loader:
                    ...
                    labels = {task: batch[idx + 1].to(device) for idx, task in enumerate(self.tasks)}# if device != self.config['data']['trainset_device'] else batch[idx + 1] for idx, task in enumerate(self.tasks)}
                    rep, mask = model['rep'](images, None)
                    for task in self.tasks:
                        out, _ = model[task](rep, None)
                        loss = loss_fn[task](out, labels[task]).item()
            ...
            if 'randsvd' in method:
                ...
                if 'direct' in method:
                    ...
                    for Ai in matrices:
                        ...
                        direct_recon += reverse_rearrange_matrix((U.to(comp_dev)@torch.diag(S).to(comp_dev)@V.to(comp_dev).T).to(comp_dev),*Ai.shape)
                    ...
                    for Ai in matrices:
                        rAi = rearrange_matrix(Ai.to(comp_dev))
                        U, S, V = randomized_svd(A = rAi, n_components = eigNb, n_oversamples = eigNb//10, niter = 2)
                        Us.append(U.to(mem_dev)); Ss.append(S.to(mem_dev)), Vs.append(V.to(mem_dev))
                    # myDel(rAi, U, S, V)
                    C_list = [ reverse_rearrange_matrix(Us[i].to(comp_dev)@torch.diag(Ss[i].to(comp_dev))@Vs[i].to(comp_dev).T, d,m).to(mem_dev)  for i in range(len(matrices)) ]
                    D_list = [(matrices[i].to(comp_dev) - C_list[i].to(comp_dev)).to(mem_dev) for i in range(len(matrices))]
                    rsumC_full = rearrange_matrix(sum(C_list).to(comp_dev))
                    U, S, V = randomized_svd(A = rsumC_full.to(comp_dev), n_components = eigNb, n_oversamples = eigNb//10, niter = 2)
                    sumC = reverse_rearrange_matrix(U@torch.diag(S)@V.T, d,m)
                    # myDel(rsumC_full)
                    term1 = sum(A_i.to(comp_dev).T @ A_i.to(comp_dev) for A_i in matrices)
                    term2 = sum(C_list[i].to(comp_dev).T@(sumC-C_list[i].to(comp_dev)) for i in range(len(matrices)))
                    term3 = 2 * sum(D_list[i].to(comp_dev).T@(sumC-C_list[i].to(comp_dev)) for i in range(len(matrices)))
                    term4 = torch.zeros_like(term3) if \
                            (sum(torch.linalg.norm(C_list[i])**2 for i in range(len(matrices))) / sum(torch.linalg.norm(D_list[i])**2 for i in range(len(matrices))) )**(0.5)>1.5\
                ...
                if 'direct' in method:
                    ...
                    C_list = [ top_k_compression(matrices[i].to(comp_dev), compression_rate).to(mem_dev)  for i in range(len(matrices)) ]
                    D_list = [(matrices[i].to(comp_dev) - C_list[i].to(comp_dev)).to(mem_dev) for i in range(len(matrices))]
