# The code is from https://github.com/dbmptr/EPOSearch
import numpy as np
import cvxpy as cp
import cvxopt
...
class EPO_LP(object):
    ...
    def __init__(self, m, n, r, eps=1e-2):
        ...
        self.nb_of_balance_dirs = 0
        self.nb_of_descent_dirs = 0

    def get_alpha(self, l, G, r=None, C=False, relax=False):
        r = self.r if r is None else r
        assert len(l) == len(G) == len(r) == self.m, "length != m"
        self.rl, self.mu_rl, self.a.value = adjustments(l, r)
        ...
        if self.mu_rl > self.eps:
            ...
            if len(np.where(J)[0]) > 0:
                J_star_idx = np.where(self.rl >= .9*np.max(self.rl))[0] # np.where(self.rl == np.max(self.rl))[0]
